...
Statement: LBRACE StatementAux RBRACE     { $$ = insertListStatement($2);}
        ...
        | RETURN Expression SEMICOLON                                   {if(error_yacc==0)$$ = insertReturn($2);}
        | RETURN SEMICOLON                                              {if(error_yacc==0)$$ = insertReturn(NULL);}
...
AuxExpr: Expression AuxExprComma                {if(error_yacc==0)$$ = insertListExpression($2, $1);}
        |                                       {if(error_yacc==0)$$ = NULL;} 
;
MethodInvocation: ID LPAR AuxExpr RPAR          {if(error_yacc==0)$$ = insertMethodInvocation($1, $3);}
                | ID LPAR error RPAR            {error_yacc = 1; $$ = NULL;}              
;
Assignment: ID ASSIGN Expression                {if(error_yacc==0)$$ = insertAssign($1, $3);}
;
ParseArgs: PARSEINT LPAR ID LSQ Expression RSQ RPAR     {if(error_yacc==0)$$ = insertParseArgs($3, $5);}
...
Expr:  TextExpr                                         {if(error_yacc==0)$$ = $1;}              
        |Expr OR Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Or", $3);}        
        |Expr AND Expr                                  {if(error_yacc==0)$$ = insertOperator($1, "And", $3);}
        |Expr LT Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Lt", $3);}
        |Expr GT Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Gt", $3);}
        |Expr EQ Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Eq", $3);}
        |Expr NE Expr                                       {if(error_yacc==0)$$ = insertOperator($1, "Ne", $3);}
        |Expr LE Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Le", $3);}
        |Expr GE Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Ge", $3);}
        |Expr PLUS Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Add", $3);}
        |Expr MINUS Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Sub", $3);}
        |Expr STAR Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Mul", $3);}
        |Expr DIV Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Div", $3);}
        |Expr MOD Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Mod", $3);}
        |Expr XOR Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Xor", $3);}
        |Expr LSHIFT Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Lshift", $3);}
        |Expr RSHIFT Expr                                   {if(error_yacc==0)$$ = insertOperator($1, "Rshift", $3);}
        |PLUS Expr %prec UNITARY                                   {if(error_yacc==0)$$ = insertUnitary("Plus", $2);}                                      
        |MINUS Expr %prec UNITARY                                   {if(error_yacc==0)$$ = insertUnitary("Minus", $2);}
        |NOT Expr                                   {if(error_yacc==0)$$ = insertUnitary("Not", $2);}
;

TextExpr: LPAR Expression RPAR                          {if(error_yacc==0)$$ = insertMultipleExpression($2);}
        |MethodInvocation                               {if(error_yacc==0)$$ = insertExpression($1, NULL, 0, NULL, NULL, NULL, NULL);}
        |ParseArgs                                      {if(error_yacc==0)$$ = insertExpression(NULL, $1, 0, NULL, NULL, NULL, NULL);}
        |ID DOTLENGTH                                   {if(error_yacc==0)$$ = insertExpression(NULL, NULL, 1, $1, NULL, NULL, NULL);}                                  
        |ID                                             {if(error_yacc==0)$$ = insertExpression(NULL, NULL, 0, $1, NULL, NULL, NULL);}
        |INTLIT                                         {if(error_yacc==0)$$ = insertExpression(NULL, NULL, 0, NULL, $1, NULL, NULL);}                                 
        |TRUE                                           {if(error_yacc==0)$$ = insertExpression(NULL, NULL, 0, NULL, NULL, "true", NULL);}
        |FALSE                                          {if(error_yacc==0)$$ = insertExpression(NULL, NULL, 0, NULL, NULL, "false", NULL);}
        |REALLIT                                        {if(error_yacc==0)$$ = insertExpression(NULL, NULL, 0, NULL, NULL, NULL, $1);}
